...
* <2020-12-17 Thu> idea to make a rofi alternative
  ...
  - https://github.com/jordansissel/xdotool/blob/master/cmd_windowfocus.c
  - https://github.com/jordansissel/xdotool/blob/master/xdo.c#L392
    xdo_focus_window

** code snippets
    #+begin_src c
bool focus_window(const Display* disp, Window wid) {
  int ret = 0;
  ret = XSetInputFocus(disp, wid, RevertToParent, CurrentTime);
  XFlush(disp);
  return ret == 0;
}
...
# eg 46137347
xdotool windowraise 127926276
xdotool windowfocus 127926276

# or
xdotool search --name "Mozilla Firefox" windowfocus
    #+end_src
...
** <2020-12-20 Sun> windows vs linux?
...
** Imgui lists etc
   - [[file:~/dev/actondev/imgui/imgui_demo.cpp::ImGui::ListBox("listbox\n(single select)", &item_current, items, IM_ARRAYSIZE(items), 4);]]
** <2020-12-26 Sat> window class?
   https://github.com/jordansissel/xdotool/blob/dd45db42f16954f22b445a2c2c928fec202314c4/xdo_search.c#L123
...
* <2020-12-18 Fri> c++ templates ??
  - https://en.cppreference.com/w/cpp/language/template_specialization
  maybe have a generic
  #+begin_src cpp
template<typename T>
s7_pointer aod::s7::ffi_object(s7_scheme* sc, T);
  #+end_src

  with default 
* TODO udate meson (55.3 -> 56)
  #+begin_src sh
pip3 install --upgrade meson
  #+end_src
* DONE <2020-12-23 Wed> mongoose http server
  https://stackoverflow.com/questions/19063975/linking-c-compiled-static-library-to-c-program
  
  #+begin_src sh :session *mongoose* :dir subprojects :shell-type vterm
...
pwd
cd ..
cd ../build/subprojects/

ls

objdump libmongoose.a
nm --demangle libmongoose.a
  #+end_src

  #+begin_src sh
ninja -C build && ./build/test/gtest-all --gtest_filter="mongoose.*"
  #+end_src

  #+begin_src sh :session *benchmark*
ab -n 1 -v 4 "http://localhost:8000/"
ab -n 1 -v 4 "https://httpbin.org/get"

ab -n 2 -v 4 "http://localhost:3000/hi"

sudo apt-get install siege

siege -d0 -r5 -c1 http://localhost:3000
siege -d0 -r10000 -c1 http://localhost:8000
curl http://localhost:8000
siege -d0 -r10000 -c1 http://localhost:8080
siege -d0 -r10000 -c1 http://localhost:8081
  #+end_src

  #+begin_src sh :session *node-echo*
npx http-echo-server
node benchmark/index
  #+end_src
* DONE <2020-12-26 Sat> gflw
  the =gui_repl_sfml= is 14.4MB. and SDL was around 13mb.

  Let's see with gflw => 11MB :)

  | gui_repl size (mb) | sfml | gflw |
  |--------------------+------+------|
  | normal             | 14.6 | 11.1 |
  | zipped             |  4.5 |  3.4 |
...
meson configure build -Dimgui:gflw='enabled'

# timing exit after draw
time ./build/src/gui_repl_sfml src/scheme/examples/window-list.scm # 0.5s
time ./build/src/gui_repl_gflw src/scheme/examples/window-list.scm # 0.25s

time rofi -show drun
  #+end_src

 
* <2020-12-26 Sat> imgui & greek strings (generally utf8)
  https://github.com/ocornut/imgui/blob/master/docs/FONTS.md
* TODO listing .desktop entries
  https://github.com/davatorium/rofi/blob/next/source/dialogs/drun.c#L878 ?
  #+begin_src sh
echo $PATH

find /var/lib/snapd/desktop/applications/ -iname "*desktop" -type f

find /usr/share/applications/ -iname "*desktop" -type f

#find / -iname "*desktop" -type f | grep Viber
find / -iname "*desktop" -type f -not -path "/media*" -not -path "/etc*"  | grep -iR viber

cat /var/lib/snapd/desktop/applications/onlyoffice-desktopeditors_onlyoffice-desktopeditors.desktop
...
grep -e Name= -e Exec= /usr/share/applications/zynaddsubfx-oss.desktop
  #+end_src
* TODO window height: match contents
  - =ImGui::GetItemRectSize()=

* TODO startup time
  gflw has a sliight delay. maybe try with SDL and software renderer? (is it the gl context creation that's causing the delay?)

  #+begin_src sh
meson configure build | grep imgui
meson configure build -Dimgui:sdl='enabled'

ninja -C build
time build/src/gui_repl_gflw examples/window-list.scm

time build/src/gui_repl_sdl examples/window-list.scm
build/src/gui_repl_gflw examples/window-list.scm
  #+end_src
* std::regex multiline
  [[file:/usr/include/c++/9/bits/regex.h::static constexpr flag_type ECMAScript = regex_constants::ECMAScript;]]

  [[file:/usr/include/c++/10/bits/regex.h::static constexpr flag_type ECMAScript = regex_constants::ECMAScript;]]

  #+begin_src sh
# nah g++-10 didn't fix it
sudo apt-get remove g++-10

sudo apt-get install clang-10

meson configure build | grep compiler
...
grep -C 2 'rule cpp_COMPILER' build/build.ninja

ninja -C build
  #+end_src

  again, in usr/include/c++/10 .. , no multiline
  https://en.cppreference.com/w/cpp/regex/basic_regex/constants
  it's there! (since c++17)
* DONE desktop entries field codes
  https://specifications.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html
  eg
  - %F
  - %u

  parse them & removed them from the =exec= field
