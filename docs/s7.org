...
(define* (ns-require the-ns (as #f) (force #f) (dynamic *ns-require-dynamic*))
  ...
  #+END_SRC
  And was wondering if upon =(set! *ns-require-dynamic* #f)= subsequent calls to =ns-require= has the updated dynamic.
  Or, if the default value is determined in the =define*= definition time, or when calling it.

  The answer: when calling it!

  So *indeed* I can =set!= the =*ns-require-dynamic*= and have its changes take effect
  on next calls to =ns-require=
